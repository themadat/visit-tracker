...
- Current version: `APP_VERSION = "4.7.5.2"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). Done: per-map `settings.graticuleByLayer` (`{us:{enabled,major:10,minor:5}, world:{enabled,major:30,minor:10}}`, defaults + `normalizeGraticuleByLayer`); a `renderGraticule()` injects a `#mapGraticule`/`#worldMapGraticule` `<g>` below the ring overlay and draws World (Robinson parallels/curved meridians + projection frame + clamped degree labels + named tropic/polar reference lines) and US (one inferred contiguous lat/lng plane fitted from state geo bounds and clipped to the stylized map, plus separate inset grids for AK/HI/territories). The header Grid button uses `__GLOBE` when off / `__GLOBE_FILL` when on and opens a non-modal `#graticuleDialog` popover (on/off + major/minor per active map); minor intervals snap to divisors of the major interval. Note: the US SVG is not a geographically projected map, so a true globe-slice graticule requires replacement projected geometry; current behavior is the best fit over existing stylized art. Lines via `.graticule-line`/`.is-major`/`.graticule-frame`/`.graticule-label` (theme-aware). Latest cut release: **4.7.4 "Crosshairs"**. WISH-061 (Rangefinder edge cases) also targets 4.7.5 but is not yet started.
