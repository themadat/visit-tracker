...
- Current version: `APP_VERSION = "4.7.5.20"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). World keeps its Robinson graticule and reference lines. Mainland and Alaska now use stable normalized geographic interpolation seeded from the prior landmark-fitted Albers frames; this removes the quadratic off-band extrapolation that doubled the southern mainland grid. Developer Mode's non-modal US Grid Tuner (Grid → Tune or Settings → Developer Tools) has separate Mainland/Alaska X, Y, Width, Height, South/North Spread, Latitude Tilt, shared South/Middle/North longitude-shape points, and South/Middle/North latitude-bow points. Live session values use `trailLog.usGridTuning.v2` (v1 intentionally retired), remain outside `usStateVisitMap.v1` and backups, and export commit-ready `US_GRATICULE_CALIBRATION` code. Grid labels, inset masks/clips, direction tinting, interval controls, and World reference behavior remain as documented in the active plan. WISH-077 remains the P0 pop-up-unification follow-up. Latest cut release: **4.7.4 "Crosshairs"**; WISH-061 also targets 4.7.5 but is not started.
