...
- Current version: `APP_VERSION = "4.5.2.5"` — active dev line **4.5.2 "Get Your Bearings"** (patch, two small features plus icon-sizing polish; no plan doc, direct line). Latest cut release: 4.5.1 "Back in the Picture". Banner target stays 4.5.0 "Leave No Trace" (patch releases skip the banner). Run `prep`/`ship` when ready.
  - **4.5.2 features:** (1) the Rangefinder ring-style ("circle options") menu stays open after each pick for live multi-select — removed the `ringStyleMenuOpen = false` line in `setRingStyleOption`; option clicks `stopPropagation` and the existing document outside-click handler (guards on `.ring-style-picker`) still closes it. (2) `renderLegend` appends a grouping icon to the **right** of `#legendTotals` (so the %/count stay aligned with the per-level rows) showing what the totals measure against: `visitTypeIcon(suggestedSetIconId(stats.set.id))` for an active Waypoint-pack label target, else `activeLayer().icon` (`__US_MAP`/`__WORLD_MAP`); styled by `.legend-grouping-icon` (16px, muted) with a fixed 14px pseudo-element divider and a 32px World-only optical adjustment. (3) the map label-target selector's base ("States") option + collapsed pill use `activeLayer().icon` + the layer heading instead of the generic `__CIRCLE_CIRCLE_FILL`; raw map SVGs are explicitly constrained to 1.35rem because their source art carries intrinsic 800px dimensions, while the World icon alone is 2.25rem in the collapsed label control. Raw map SVGs use `fill: currentColor` in these controls so they follow the active theme.
...
- Latest public releases (newest first): 4.5.1 "Back in the Picture", 4.5.0 "Leave No Trace", 4.4.4 "Fine Print", 4.4.3 "Clear View", 4.4.2 "True Colors", 4.4.1 "Ultralight", 4.4.0 "Basecamps". Full per-release behavior lives in the `CHANGELOG` constant in `assets/js/changelog.js`; release table in `README.md`. Don't duplicate per-release prose here — read the CHANGELOG entry for the version in question.
