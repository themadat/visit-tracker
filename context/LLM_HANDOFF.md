...
- Current version: `APP_VERSION = "4.6.0.3"` — active dev line **4.6.0 "The Whole Story"**. Plan: `context/4.6.0-DETAILED-NOTES-PLAN.md`. Latest cut release: 4.5.2 "Get Your Bearings".
  - **4.6.0 in progress:** Notes has a fourth `detailed` view before Expanded, using `__RECTANGLE_EXPAND_DIAGONAL`. Fresh/default settings use Detailed while existing saved view modes remain valid. Detailed cards use a location-precision/level/tag header, natural Location/Event phrases, directly editable priority, inline Details editing with a 2,000-character limit, and linked-Waypoint website/photo/priority actions in both Notes contexts. Coordinates are intentionally omitted. Down Arrow cycles views, Up Arrow cycles sorts, and Back restores the main list's scroll position. The same four-view selector renders in a sticky individual-location Visit Notes header and shares the persisted `notesViewMode`.
