...
- Open follow-ups: WISH-080 (Target Finder Maps/Flights, P0 — **in dev on the 4.9.0 line**, see `context/WISH-080-TARGET-FINDER-MAPS-PLAN.md`); WISH-079 (mobile center-crosshair Rangefinder, P1); WISH-076 (native-language World labels, P1); WISH-073 (Basecamp photo support, P2).
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. **Active dev line: `context/WISH-080-TARGET-FINDER-MAPS-PLAN.md` (4.9.0 "Travel Agent")** — implemented at `APP_VERSION = "4.9.0.1"`: summary-row re-wire (compare readout is now the Local Times opener `data-ring-timezone-toggle`; the right `.ring-maps-btn` keeps a consistent icon `__POINT_TOPLEFT_DOWN_TO_POINT_BOTTOMRIGHT_CURVEPATH` and opens the handoff — single click `openRingExternal(false)` = focused new tab, right-click = `openRingExternal(true)` background tab); provider toggle in Settings → Map Interaction carries the `__APPLE_LOGO`/`__G_SQUARE_FILL` logos via `setButtonLabelBoth` in `syncButtonLabels()` (`settings.mapsProvider`, platform-aware default, normalized to apple|google); Drive = provider directions, Plane = Google Flights, one-point = provider place; URL builders + `ringExternalLinks()` near `ringCompareReadout()`. Remaining for `prep`/`ship`: Help/FAQ, hints, README, lock theme/cta, mark WISH-080 done. Queued plan: `context/WISH-063-THEMING-OVERHAUL-PLAN.md` (app-wide theming overhaul; 5.0.0 major flagship).
