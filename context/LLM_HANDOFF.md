...
- Current version: `APP_VERSION = "4.7.1.1"` — active development line **4.7.1 "Atlas Ink"** for WISH-057 "Smarter World Map Labels"; plan in `context/WISH-057-WORLD-MAP-LABELS-PLAN.md` (declutter + shrink-to-fit, leader-line microstate labels off the `.circlexx` dots, and English short names; world map only).
...
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. Active plan: `context/WISH-057-WORLD-MAP-LABELS-PLAN.md` (WISH-057, active at 4.7.1.1). Queued plan: `context/WISH-071-LATLNG-GRATICULE-PLAN.md` (WISH-071 lat/lng graticule, target 4.7.2, not yet started).
