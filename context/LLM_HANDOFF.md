...
- Current version: `APP_VERSION = "4.5.2.1"` — active dev line **4.5.2 "Get Your Bearings"** (patch, two small features; no plan doc, direct line). Latest cut release: 4.5.1 "Back in the Picture". Banner target stays 4.5.0 "Leave No Trace" (patch releases skip the banner). Run `prep`/`ship` when ready.
  - **4.5.2 features:** (1) the Rangefinder ring-style ("circle options") menu stays open after each pick for live multi-select — removed the `ringStyleMenuOpen = false` line in `setRingStyleOption`; option clicks `stopPropagation` and the existing document outside-click handler (guards on `.ring-style-picker`) still closes it. (2) `renderLegend` prepends a grouping icon to `#legendTotals` showing what the totals measure against: `circleIconSvg(suggestedSetIconId(stats.set.id))` for an active Waypoint pack, else `activeLayer().icon` (`__US_MAP`/`__WORLD_MAP`); styled by `.legend-grouping-icon` (16px, muted, right divider).
