...
- Current version: `APP_VERSION = "4.7.5.5"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). Done: per-map `settings.graticuleByLayer` (`{us:{enabled,references,labels,major:10,minor:5}, world:{enabled,references,labels,major:30,minor:10}}`, defaults + `normalizeGraticuleByLayer`); a `renderGraticule()` injects a `#mapGraticule`/`#worldMapGraticule` `<g>` below the ring overlay and draws World (Robinson parallels/curved meridians + projection frame + clamped degree labels + optional named tropic/polar reference lines) and US (U.S. National Atlas Equal Area graticule over 50°N–25°N and 125°W–65°W, affine-fitted to lower-48 SVG geometry, with an SVG mask hiding the grid inside AK/HI/territory inset cutouts without covering separator strokes). The header Grid button uses larger `__GLOBE` when off / `__GLOBE_FILL` when on and toggles a non-modal `#graticuleDialog` popover clamped inside `.map-panel`: Grid (`__APP_GRID`), Circles (`__CIRCLE_DASHED`), Labels (`ICON_SETTINGS_SHORTCUTSON__TAG_FILL`), Major, Minor in one compact row. Circles/Labels/Major/Minor disable while Grid is off. Major/Minor app-side steppers avoid browser-native number jumps; Minor intervals snap to divisors of the major interval. Lines via `.graticule-line`/`.is-major`/`.graticule-frame`/`.graticule-label` (theme-aware). WISH-077 is a P0 follow-up for unified anchored control pop-ups. Latest cut release: **4.7.4 "Crosshairs"**. WISH-061 (Rangefinder edge cases) also targets 4.7.5 but is not yet started.
