...
- Current version: `APP_VERSION = "4.5.1"` — latest cut release **4.5.1 "Back in the Picture"** (patch, shipped 2026-06-12). No active dev line; the next substantial change starts with `plan` (or `start` when a plan already exists). Newest *feature* entry (banner target) is still 4.5.0 "Leave No Trace" — patch releases skip the banner.
