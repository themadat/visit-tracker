...
- Current version: `APP_VERSION = "4.7.4.4"` — active dev line **4.7.4 "Crosshairs"**. **Done:** WISH-074 pinch-to-zoom (4.7.4.1) plus gesture bugfixes (4.7.4.2/4.7.4.3/4.7.4.4): touch two-finger pinch snaps to `MAP_ZOOM_STOPS`; two-finger tap zooms out; double-tap/double-click zoom-in now works from Fit mode and desktop pointers; laptop trackpad pinch uses wheel/gesture paths; `.map-wrap` keeps native vertical touch scroll smooth while scroll mode allows native horizontal pan (`touch-action: pan-x pan-y`) and the custom pan lock waits for clear horizontal intent before grabbing touch movement. **Next on this line:** WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`; not yet started — Robinson world graticule via `worldCoordinatePoint`/`worldProjectionBox` ≈6404/6420, per-state US grid via `LOCATION_GEO_BOUNDS`≈1727 / `LOCATION_PROJECTION_FRAME_OVERRIDES`≈1751 / `projectNoteLocation`≈6438, overlay `<g>` like the ring overlay ≈5536, settings `graticuleByLayer` mirroring `ringByLayer`). Latest cut release: **4.7.3 "Camera Trail"**.
