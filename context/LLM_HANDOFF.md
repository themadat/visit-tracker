...
- Write or revise `context/WISH-<id>-<slug>-PLAN.md` with scope, constraints,
  ...
  concrete test plan.
- When a Roadmap seed has a written plan, set its optional `planDoc` field to
  the plan filename so the Roadmap ticket pill can mark it as planned.
...
- Keep `.github/workflows/deploy.yml`'s fixed `name` at `Deploy Trail Log v<APP_VERSION>` on every version change, including `ship`; `./build/check.sh` checks alignment. Commit subjects use `Version - Text`; the workflow's dynamic run title adds the channel and commit message, with the versioned name as the manual-run fallback.
- Deployment: `_vt-checkpoint` commits/pushes the feature branch and pushes HEAD to beta. Already committed? Use `_vt-deploy beta` (checkpoint exits at an empty commit). Feature-branch pushes alone do not deploy. All main/beta/alpha workflows must share concurrency `group: pages`, `cancel-in-progress: false`, `queue: max`, because they write the same `gh-pages` branch. Roll the fix into all channels; avoid overlapping deployments while old channel workflows remain. Preserve the existing branch-based Pages source (`gh-pages`, root) and channel folders. See README's Checkpoint and deploy flow.
...
- Current version: `APP_VERSION = "5.1.1.2"` — active patch **Clear Signal**, adding the supplied Data Sync braces icon and useful browser-network failure messages. Latest cut: **5.1.0 Cloud Trail**, optional GitHub data sync from the sibling app-template. The combined local/cloud status action is first in the top-bar toolbar; Settings → Data Sync hosts setup, explicit transfer, conflict choices, cloud restore, recovery and payload preview. Fixed target: `themadat/app-data/main/data/trail-log.json`. Small direct patch; no plan doc. Found a concrete EasyPrivacy false-positive match: its /visit-tracker.js rule also matches the old .json filename. 5.1.1.2 uses data/trail-log.json for reads/writes and instructs existing cloud users to rename the old file before syncing. Old target baselines must not carry over; local state and secrets stay intact. No real credentials read or tested.
- Sync lives in classic `assets/js/sync{,-data,-storage,-ui,-icons}.js` companions. `initTrailLogSync` bridges the main `save()` and state/render callbacks. Content envelope `trail-log-data` v1 includes map name, legend definitions, pack-icon mapping (managed note metadata), US/World levels and notes, and Basecamp pads/links. General preferences, legend colors, tag configuration, UI/release state, credentials and bookkeeping stay local. Sync downloads use existing normalization and reject content that would be lost. `usStateVisitMap.v1` remains the full local/backup format.
- Sync metadata/recovery/secrets use separate `trailLog.sync.v1`, `trailLog.syncRecovery.v1`, `trailLog.githubToken.v1` and `trailLog.githubToken.session.v1` keys. Fixed-target defaults/repair live in `createTrailLogSyncStorage`; credentials are never part of the app state, sync payload, developer JSON or ordinary backups. Background checks are read-only; initial/conflicting writes require choices, and downloads/merges require successful recovery. Do not test against real tokens or user cloud data. `tests/sync-browser.html` isolates storage and the GitHub API in a test iframe.
...
- Latest public releases (newest first): 5.1.0 "Cloud Trail", 5.0.0 "Paint Job", 4.9.0 "Travel Agent", 4.8.1 "Reading Glasses", 4.8.0 "Square Deal", 4.7.6 "Dead Reckoning", 4.7.5 "Gridlines", 4.7.4 "Crosshairs", 4.7.3 "Camera Trail", 4.7.2 "Trail Marks", 4.7.1 "Atlas Ink", 4.7.0 "Priority", 4.6.1 "A Small World", 4.6.0 "The Whole Story", 4.5.2 "Get Your Bearings", 4.5.1 "Back in the Picture", 4.5.0 "Leave No Trace", 4.4.4 "Fine Print", 4.4.3 "Clear View", 4.4.2 "True Colors", 4.4.1 "Ultralight", 4.4.0 "Basecamps". Full per-release behavior lives in the `CHANGELOG` constant in `assets/js/changelog.js`; release table in `README.md`. Don't duplicate per-release prose here — read the CHANGELOG entry for the version in question.
- Recent shipped scope (one-liners only; see CHANGELOG for detail):
  - 5.0.0 **Paint Job** — app-wide theming (WISH-063). Top-bar `#paletteBtn` opens a column-aligned Theme & Colors grid (header Main · Wayfinder · Rangefinder · Levels · Priorities; editable Current row of swatch+hex cells; 13 `COLOR_THEMES` rows that highlight when matched). Edits the flat `settings.palette` CSS-var bag (accent + accent-2 derived; 5 priorities; 2 ring endpoints → 8 stroke steps + fills via `color-mix` + connector; wayfinder + strong/deep derived) plus per-level `levels[].color`. Key symbols: `THEMEABLE_VARS`, `COLOR_THEMES`, `DEFAULT_THEME` (pinned to Classic by id), `normalizePalette`/`normalizePaletteHex`, `applyPaletteOverrides`, `derive{Accent,Wayfinder,Ring}Vars`, `paletteVarValue`, `editPaletteSwatch`, `applyColorTheme`, `levelPaletteAssignment`/`assignLevelPalette`, `currentMatchesTheme`, `renderPaletteDialog`/`bindPaletteGrid`. CSS `#paletteDialog` + `.palette-grid*` in `app.css`. Ring `--ring-step-N-fill` derive from the stroke vars (`color-mix`), so themes recolor lines + fills. Surfaces/contrast-badges intentionally excluded. `usStateVisitMap.v1` unchanged.
  - 4.9.0 **Travel Agent** — Rangefinder Target Finder external handoff (WISH-080). The summary row's compare readout became the Local Times & Arrival opener (`data-ring-timezone-toggle`), and a new `.ring-maps-btn` (consistent icon `__POINT_TOPLEFT_DOWN_TO_POINT_BOTTOMRIGHT_CURVEPATH`) opens an external trip tool: Drive → preferred-provider directions, Plane → Google Flights search, one point → provider place lookup. Single click = focused new tab (`openRingExternal(false)`), right-click = background tab (`openRingExternal(true)` via a ctrl/cmd synthetic anchor click; native menu suppressed). Key symbols: `ringExternalLinks`, `google/appleDirectionsUrl`, `google/applePlaceUrl`, `googleFlightsUrl`, `renderRingMapsButton` near `ringCompareReadout`. Provider picked in Settings → Map Interaction (logos via `setButtonLabelBoth` in `syncButtonLabels()`). Adds persisted `settings.mapsProvider` (`apple`|`google`, platform-aware default in `defaultState()`, coerced in `normalizeState()`); `usStateVisitMap.v1` schema otherwise unchanged.
  - 4.8.1 **Reading Glasses** — the Text Only text size set with Smaller/Larger (or `{`/`}`) now persists. Adds `settings.notesTextOnlyFontPx` (default in `defaultState()`, rounded+clamped to 9–22px in `normalizeState()`); `usStateVisitMap.v1` schema otherwise unchanged.
  - 4.8.0 **Square Deal** — consistency/polish pass (WISH-077). Anchored pop-ups (Priority, Grid, Rangefinder style/time, Legend position, Map/Waypoint label pickers, Waypoint priority/link, Basecamp popovers) unified into one square-control system with shared sizing, states, and on-screen clamping; primary 36px / inset 32px / framed 40px control-height system; Wayfinder dropped from the Map (Notes-only); Notes header top-aligns, search-bar height matched, resize handle unified; Rangefinder endpoint labels wrap in fixed slots, map header stays top-aligned across modes, side-column Rangefinder adapts at 40% width. Text Only gained a scroll-sticky Smaller/Larger/Copy cluster floating over the text (session-only `notesTextOnlyFontPx`, 9–22px, icons `__TEXT_BADGE_MINUS`/`__TEXT_BADGE_PLUS`) and now starts text at the top. Keyboard shortcuts fully revised (map `|`/`:`/`9`–`6`/`'`/`←`/`~`; Legend/Notes W/X/Y/Z, F/R, `>`; Rangefinder E/J/K/U/V, T; numbered pop-ups; `{`/`}` font size). No `usStateVisitMap.v1` schema changes.
  - 4.7.6 **Dead Reckoning** — Rangefinder edge-case/globe-wrap/time-zone line (WISH-061 + WISH-078). Cross-region rings project into every reached US region (mainland↔inset, inset↔inset) regardless of End, with full-depth graduated shading and inset-frame-bounded dotted cues; World rings trace true great-circles sampled to match Robinson and wrap at the curved date-line seam; distance/time render in collision-aware rangefinder-colored pills sampled along curves with a dedicated top layer when crowded; offline IANA time zones drive local clocks, departure→destination-local arrival, signed time-zone deltas, and a compact inset time planner. Key symbols: `renderRingOverlay`, `worldGeodesicRingSegments`, `appendInsetRingCoverage`, `appendCrossRegionRingArcs`, `ringZoneChangeText`; data in `assets/js/timezones.js`. Adds `settings.ringByLayer.*` time-zone fields (`timeZoneOpen`/`departureMode`/`departureLocal`/`zoneOverrides`); `usStateVisitMap.v1` schema otherwise unchanged.
  - 4.7.5 **Gridlines** — US and World maps gain optional Major/Minor latitude/longitude grids, Robinson reference circles and frame, calibrated Mainland/Alaska/Hawaii inset geometry, shared Mainland/Alaska coordinate-pin projection, and a Developer Tools-only nine-point US grid tuner. Adds persisted `settings.graticuleByLayer`; `usStateVisitMap.v1` remains unchanged.
  - 4.7.4 **Crosshairs** — scrollable maps gain touch pinch zoom, double-tap/double-click zoom-in, two-finger tap zoom-out, trackpad pinch support, stable small-screen zoom layout, and smoother two-axis mobile panning. No schema changes.
  - 4.7.3 **Camera Trail** — Waypoint Pack photo previews derive a Commons file name, lazily check Commons for camera-location/EXIF GPS metadata, and render a small camera-spot caption under the photo in the note editor, Waypoint Packs panel, and Detailed note cards. The caption links to Google Maps, shows heading/offset when available, and uses muted checking/unavailable/error states. In-memory caches only; no persisted schema change and note lat/lng are never overwritten.
  - 4.7.2 **Trail Marks** — Roadmap cards accent ticket pills for wishes with `planDoc`, planned wishes are searchable by "plan", roadmap metadata fits on one compact line, Oldest/Newest sorts by ticket number, Full Update List sections render as clean headers with single-level bullets, and a repeatable local-only raptor easter egg hides on the US map (developer trigger: Niihau/westernmost Hawaii island). No persisted app-state schema changes.
...
- Open follow-ups: WISH-079 (mobile center-crosshair Rangefinder, P1); WISH-076 (native-language World labels, P1); WISH-073 (Basecamp photo support, P2).
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. No active plan docs.
...
- User data lives in browser localStorage. Locate, Waypoint Pack Wikipedia
  ...
  online actions and only run when clicked. Optional GitHub Sync checks after setup; transfers remain explicit.
...
- Preserve offline/local-first use. Optional GitHub Sync is explicitly authorized and opt-in; it checks only after connection setup and transfers via Sync Now/Restore. Other destructive UI actions go through `requestConfirm(...)`.
...
- Item shape: `title`, `ticketId` (`WISH-###`), optional `planDoc`, `description`, `priority`, `effort`, `targetKind`, `targetVersion`, `tokenCostPct`, `prompt`, `category`.
...
- Legend: `renderLegend`, `moveLevel`, `deleteLevel`, `assignLevelPalette`, `setLegendPosition`.
- Theme & Colors (WISH-063): `COLOR_THEMES`/`THEMEABLE_VARS`/`DEFAULT_THEME`, `normalizePalette`, `applyPaletteOverrides`, `derive{Accent,Wayfinder,Ring}Vars`, `editPaletteSwatch`, `applyColorTheme`, `currentMatchesTheme`, `renderPaletteDialog`/`bindPaletteGrid`.
...
```js
{
  appVersion, mapName,
  settings: {
    ..., theme, palette, activeLayerId, mapLabels,   // palette: flat {"--var":"#hex"} theme overrides
...
- Data Sync: combined local/cloud status at the left of top-bar actions. Settings → Data Sync includes the pinned GitHub target, masked device/tab token, Test/Save/Forget, Sync Now/Restore from Cloud, local recovery restore/export, and safe text-only JSON preview. Mirrors the template’s fourteen cloud states; no floating control.
...
- Theme & Colors: top-bar `#paletteBtn` (left of Settings, `>` shortcut) opens a column-aligned grid — header (Main · Wayfinder · Rangefinder · Levels · Priorities), an editable Current row (a swatch + hex field per cell, settable by picker or typed hex), and 13 one-click theme rows that highlight when the current colors match. Recolors legend level colors, the 5 note priorities, the 2 Rangefinder ring endpoints, the Wayfinder accent, and the app accent; companion shades (ring ramp + fills + connector, accent-2, Wayfinder strong/deep) derive automatically. Persists the flat `settings.palette` CSS-var bag, applied in both light/dark; rides along in JSON backups. App surfaces and contrast badges are intentionally out of scope.
- Rangefinder: Shortcut Mode key `5` / target button opens a paired panel with the Legend, picks saved note pins as Start/End, draws straight-line Drive/Plane planning rings on US and World maps, uses compass-correct cross-inset cues plus geodesic ring arcs in the End region, limits inset-start radii, wraps date-line rings/cues at the curved Robinson outline, and keeps per-map ring distances, units, fill/clip/time style, travel mode, average speed, offline zones, and departure/arrival settings. The summary row's compare readout opens Local Times & Arrival; the adjacent `.ring-maps-btn` hands Start/End to the preferred maps provider (Drive directions / Plane Google Flights / single-place fallback), single click = focused new tab, right-click = background tab; provider is set in Settings → Map Interaction (`settings.mapsProvider`).
