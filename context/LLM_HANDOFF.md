...
- Current version: `APP_VERSION = "4.7.5.19"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). World keeps its Robinson graticule and reference lines. The US layer has independent mainland, Alaska, and Hawaii grids; mainland and Alaska now derive from their fitted Albers geometry but constrain every longitude to a straight north/south chord while applying one consistent adjustable curve factor to latitude lines. Developer Mode exposes a non-modal US Grid Tuner from Grid → Tune or Settings → Developer Tools, with separate Mainland/Alaska X, Y, Width, Height, and Curve controls, live rendering, reset actions, session-only persistence at `trailLog.usGridTuning.v1`, and commit-ready `US_GRATICULE_CALIBRATION` code output. The tuning key is deliberately outside `usStateVisitMap.v1` and ordinary JSON backups. Grid labels, inset masks/clips, direction tinting, interval controls, and World reference behavior remain as documented in the active plan. WISH-077 remains the P0 pop-up-unification follow-up. Latest cut release: **4.7.4 "Crosshairs"**; WISH-061 also targets 4.7.5 but is not started.
