...
- Current version: `APP_VERSION = "4.4.5.2"` — active dev line **4.4.5 "Mobile Cleanup"**, **all 4 fixes implemented** (small direct line, no plan doc; see Resume below). Ready for `prep`/`ship` once reviewed. Latest cut release: 4.4.4 "Fine Print".

### Resume — 4.4.5 "Mobile Cleanup" (4/4 done 2026-06-11)

Four mobile fixes requested; **all 4 done**. App parses clean (`./build/check.sh`); line at `4.4.5.2` with a `CHANGELOG` "Mobile Cleanup" entry (4 highlights, 4 Mobile update items). Patch release (patch=5) so no banner/cta — `latestFeatureNotice()` correctly returns 4.4.0. Next step is `prep`/`ship`. Verify with `python3 -m http.server 8018` + `preview_resize` mobile (375px); **navigate plain `…/index.html`, no `?cb=` query (it lands on chrome-error here)**; resize first then re-navigate (resize reloads the wrapper's broken URL); force CSS reload by swapping the `app.css` link href (browser caches assets).

- **DONE — Tip Jar double scroll.** Base `dialog` capped height with `100vh` while `#tipJarDialog .dialog-body` used `100dvh`; URL-bar gap let both scroll. Fix in `app.css` `#tipJarDialog`: grid column (`auto minmax(0,1fr)`), `overflow:hidden`, `max-height: min(860px, calc(100dvh - 1.6rem))`.
- **DONE — Map controls misaligned in scroll view.** In scroll mode `.map-control-row` centers the zoom pill + 34px Fit toggle as one group, so the pill sat left-of-center while the notes-pin/label rows below center at the viewport midline; Fit mode hides the pill (`[data-map-mode="fit"] .map-zoom-controls{display:none}`) and re-centers the lone toggle. Fix: scroll-only leading spacer mirroring the Fit toggle so the pill centers in line with the rows below — `app.css` in `@media (max-width:620px)`, `[data-button-style="symbols"][data-map-mode="scroll"] .map-control-row:has(.map-zoom-controls)::before { flex:0 0 34px }`.
- **DONE — Auto Colors / Smart Color Swatches modal.** Ask (clarified) was: in the `#paletteDialog` ("Smart Color Swatches") rows, the color swatches and the Apply check button should share one line on mobile. The `@media (max-width:620px)` `.theme-row { grid-template-columns:1fr }` was stacking name/swatches/Apply on three lines. Fix: `.theme-row` → `minmax(0,1fr) auto`, `.theme-row > b { grid-column:1/-1 }` (name on its own line), `.theme-row .swatch-grid { grid-template-columns: repeat(5, minmax(0,1fr)) }` so swatches shrink to fit beside Apply.
- **DONE — Basecamp fits mobile screen.** Root cause was stale CSS, not scroll sizing: the `@media (max-width:720px)` `.basecamp-sidebar` override targeted `.basecamp-search-wrap` / `.basecamp-new-wrap` as direct grid children, but the current markup nests the search input + New Pad button inside `.basecamp-search-row` (and `.basecamp-new-wrap` no longer exists), collapsing the search input behind the New Pad button and overlapping the hints. Fix: single-column stack (`.basecamp-sidebar { grid-template-columns: minmax(0,1fr); grid-template-rows: auto auto auto }`), `.basecamp-pad-list { grid-row:3 }` horizontal strip. Verified: search input full-width, no overlap, dialog fits 375px viewport, only the contenteditable scrolls (no double-scroll).
