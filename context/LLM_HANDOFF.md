...
- Current version: `APP_VERSION = "4.7.4"` — latest cut release **4.7.4 "Crosshairs"**. Shipped WISH-074 touch/desktop map zoom gestures: touchscreen pinch snaps to `MAP_ZOOM_STOPS`, double-tap/double-click zooms in from Fit or Scroll mode without selecting the SVG, two-finger tap zooms out, laptop trackpad pinch uses wheel/gesture paths, mobile vertical page scroll stays native, horizontal map pan remains available, and small-screen scroll mode locks the map viewport height while zoom grows pannable content. **Next likely line:** WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`; not yet started — Robinson world graticule via `worldCoordinatePoint`/`worldProjectionBox` ≈6404/6420, per-state US grid via `LOCATION_GEO_BOUNDS`≈1727 / `LOCATION_PROJECTION_FRAME_OVERRIDES`≈1751 / `projectNoteLocation`≈6438, overlay `<g>` like the ring overlay ≈5536, settings `graticuleByLayer` mirroring `ringByLayer`).
