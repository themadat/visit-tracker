...
- Current version: `APP_VERSION = "4.7.3.1"` — active dev build for **WISH-075 "Show a Pack Photo's Camera Location"** targeting 4.7.3. Latest shipped public patch remains **4.7.2 "Trail Marks"**.
...
- Latest public releases (newest first): 4.7.2 "Trail Marks", 4.7.1 "Atlas Ink", 4.7.0 "Priority", 4.6.1 "A Small World", 4.6.0 "The Whole Story", 4.5.2 "Get Your Bearings", 4.5.1 "Back in the Picture", 4.5.0 "Leave No Trace", 4.4.4 "Fine Print", 4.4.3 "Clear View", 4.4.2 "True Colors", 4.4.1 "Ultralight", 4.4.0 "Basecamps". Full per-release behavior lives in the `CHANGELOG` constant in `assets/js/changelog.js`; release table in `README.md`. Don't duplicate per-release prose here — read the CHANGELOG entry for the version in question.
- Active dev scope: 4.7.3.1 **Camera Trail** — Waypoint Pack photo previews derive a Commons file name, lazily check Commons for camera-location/EXIF GPS metadata, and render a small camera-spot caption under the photo in the note editor, Waypoint Packs panel, and Detailed note cards. The caption links to Google Maps, shows heading/offset when available, and uses muted checking/unavailable/error states. In-memory caches only; no persisted schema change and note lat/lng are never overwritten.
