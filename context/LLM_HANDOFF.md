...
- Current version: `APP_VERSION = "4.7.5.24"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). World keeps its Robinson graticule and reference lines. Mainland and Alaska use a ruled 3×3 control surface: straight longitude lines between north/south endpoints and curved latitude rows through west/center/east points. Developer Mode's non-modal US Grid Tuner, available only from Settings → Developer Tools, draws nine draggable handles directly on the map: mainland 30°N/40°N/50°N × 120°W/100°W/80°W, Alaska 55°N/60°N/65°N × 160°W/150°W/140°W. Middle-row handles move their whole straight longitude; clipped points get edge proxy handles; the panel minimizes to keep the map reachable on phones. The user-fitted Mainland and Alaska layouts are baked into `US_GRATICULE_CALIBRATION`, and coordinate-based National Park previews, saved-note markers, clusters, and Rangefinder anchors use that same projection; Hawaii and territories retain their inset-specific paths. Tuner changes refresh pins live. Live session values use `trailLog.usGridTuning.v3`, remain outside `usStateVisitMap.v1` and backups, and export commit-ready calibration code. Grid labels, inset masks/clips, direction tinting, interval controls, and World reference behavior remain as documented in the active plan. WISH-077 remains the P0 pop-up-unification follow-up. Latest cut release: **4.7.4 "Crosshairs"**; WISH-061 also targets 4.7.5 but is not started.
