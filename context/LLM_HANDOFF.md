...
- Current version: `APP_VERSION = "4.4.0.23"` — active development line is 4.4.0 "Basecamps"; latest cut release remains 4.3.0 "Waypoint Packs".
...
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. Active plan: [WISH-053 — Multiple Named Basecamp Pads with Rich Text](WISH-053-basecamp-pads-PLAN.md) (4.4.0 "Basecamps" line, branch `4-4-0-basecamps`). Active development is at `4.4.0.23`; `prep` and `ship` have not run.
