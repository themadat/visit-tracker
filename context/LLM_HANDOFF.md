...
- Current version: `APP_VERSION = "4.4.0.29"` — active development line is 4.4.0 "Basecamps" (prep complete, awaiting `ship`); latest cut release remains 4.3.0 "Waypoint Packs".
- 4.4.0 ships **Basecamps**: Basecamp grows from a single scratchpad into up to twenty named pads, each with its own icon, sanitized rich-text HTML, derived plain text, timestamps, and links to existing US/World notes. The dialog has a responsive pad sidebar with select/reorder-only rows, search-across-pads with ephemeral match highlights, inline rename + icon picker on the editor header, drag and `Alt+Up`/`Alt+Down` reorder, and dockable/full layouts. The icon-first Format/Font/Text/Other toolbars cover headings, bullet/numbered/dashed/check lists, emphasis/highlight/shadow/double-decoration, sizing, baseline, alignment, justification, quote levels, indent, link, code block, and clear formatting, with Cmd/Ctrl+Z/Y editor-local undo/redo and Cmd/Ctrl shortcut chips. Linked-note search/open/unlink flows leave Basecamp open behind the editor; per-pad and all-pad Copy and per-pad Markdown/RTF/Plain Text export sections also ship. `usStateVisitMap.v1` is unchanged; old `{ text, updated }` 4.3.0 Basecamp backups migrate once into the first pad titled "Basecamp Pad" with the default Basecamp pad icon and preserved timestamps, and the migration never overwrites pads that already exist. The Basecamps line also fixed World Locate scoping/projection, removed Basecamp's per-keystroke DOM normalization/full-document parsing, dropped offscreen plain-text layout, and hid Developer JSON rendering. WISH-071 (optional lat/lng map lines) is now P0; WISH-073 (P2 Basecamp photo support) is queued as the post-4.4.0 follow-up.
...
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. Active plan: [WISH-053 — Multiple Named Basecamp Pads with Rich Text](WISH-053-basecamp-pads-PLAN.md) (4.4.0 "Basecamps" line, branch `4-4-0-basecamps`). Active development is at `4.4.0.29`; `prep` is complete, `ship` has not run.
...
- Basecamp: up to twenty named rich-text pads with dedicated icons, sidebar select/reorder-only rows with last-updated stamps, drag plus `Alt+Up`/`Alt+Down` reorder, header-click rename and icon picker, cross-pad search (focusable via `/`), icon-first Format/Font/Text/Other toolbars with Cmd/Ctrl shortcut chips and editor-local Cmd/Ctrl+Z/Y undo/redo, safe URL/paste handling, linked existing US/World notes (open/unlink, Basecamp stays open behind the editor), per-pad and all-pad copy, and per-pad Markdown/RTF/Plain Text export sections. Legacy 4.3.0 `{ text, updated }` migrates once into a "Basecamp Pad" pad with the default icon.
