...
- Current version: `APP_VERSION = "4.7.5.18"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). Done: per-map `settings.graticuleByLayer` (`{us:{enabled,references,labels,major:10,minor:5}, world:{enabled,references,labels,major:30,minor:10}}`, defaults + `normalizeGraticuleByLayer`); a `renderGraticule()` injects a `#mapGraticule`/`#worldMapGraticule` `<g>` below the ring overlay and draws World (Robinson parallels/curved meridians + projection frame + clamped degree labels + named Equator/tropic/polar reference lines labeled on both globe edges) and US (a flatter contiguous-US Albers-style graticule using a 35°/36° conic and landmark fit so 25°N meets Texas/Florida, 49°N follows the western/north-central border, Maine's northern tip meets 47°N, and 125°W–67°W reaches the coastal extremes; a wider 60°N–0° / 145°W–45°W envelope keeps lines running off all four SVG edges; an SVG mask follows the map-separator geometry around AK/HI/territory inset cutouts; Alaska uses its own Alaska Albers projection with 55°/65° standard parallels, shifts the grid about 0.5° north, aligns 141°W to the nearly vertical Canadian border before the panhandle flares east, and draws from a wider 40°N–85°N / 180°W–110°W envelope so clipping reaches every separator edge; Hawaii keeps its independent Mercator inset grid). Inset grid line subgroups carry the pane clip-path; inset labels are un-clipped siblings, anchored from visible clipped line endpoints, and guarded against corner collisions so glyphs are not chopped or pushed into neighboring panes. The header Grid button uses larger `__GLOBE` when off / `__GLOBE_FILL` when on and toggles a non-modal `#graticuleDialog` popover clamped inside `.map-panel`: Grid (`__APP_GRID`), Circles (`__CIRCLE_DASHED`), Labels (`ICON_SETTINGS_SHORTCUTSON__TAG_FILL`), Major dropdown (5° steps through 90°), Minor dropdown (valid divisors of Major). Circles/Labels/Major/Minor disable while Grid is off. US labels attach to visible top/bottom/left/right line endpoints with corner guards; inset labels use the same label toggle at a smaller size. Degree labels are lightly direction-tinted (`N` red, `S` green, `E` orange-yellow, `W` blue-purple) via `.graticule-label.is-*`; named reference labels keep their accent styling. Lines via `.graticule-line`/`.is-major`/`.graticule-frame`/`.graticule-label` (theme-aware). WISH-077 is a P0 follow-up for unified anchored control pop-ups. Latest cut release: **4.7.4 "Crosshairs"**. WISH-061 (Rangefinder edge cases) also targets 4.7.5 but is not yet started.
