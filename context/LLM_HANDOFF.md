...
- Current version: `APP_VERSION = "4.7.5.4"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). Done: per-map `settings.graticuleByLayer` (`{us:{enabled,references,labels,major:10,minor:5}, world:{enabled,references,labels,major:30,minor:10}}`, defaults + `normalizeGraticuleByLayer`); a `renderGraticule()` injects a `#mapGraticule`/`#worldMapGraticule` `<g>` below the ring overlay and draws World (Robinson parallels/curved meridians + projection frame + clamped degree labels + optional named tropic/polar reference lines) and US (U.S. National Atlas Equal Area graticule over 50°N–25°N and 125°W–65°W, with cover patches hiding the grid behind AK/HI/territory inset cutouts). The header Grid button uses larger `__GLOBE` when off / `__GLOBE_FILL` when on and toggles a non-modal `#graticuleDialog` popover clamped inside `.map-panel`: Grid (`__APP_GRID`), Circles (`__CIRCLE_DASHED` if available, fallback `__CIRCLE_DOTTED_CIRCLE`), Labels (`ICON_SETTINGS_SHORTCUTSON__TAG_FILL`), Major, Minor in one compact row. Circles/Labels disable while Grid is off. Minor intervals snap to divisors of the major interval; ArrowUp/ArrowDown and native number steppers jump across skipped divisors. Lines via `.graticule-line`/`.is-major`/`.graticule-frame`/`.graticule-label` (theme-aware). Latest cut release: **4.7.4 "Crosshairs"**. WISH-061 (Rangefinder edge cases) also targets 4.7.5 but is not yet started.
