...
- Current version: `APP_VERSION = "5.0.0"` — latest cut release is **5.0.0 "Paint Job"** (WISH-063): a top-bar Theme & Colors panel (`#paletteBtn`, left of Settings, `>` shortcut) recolors legend levels, the 5 note priorities, the 2 Rangefinder ring endpoints (8-step stroke ramp + derived fills + connector), Wayfinder (strong/deep derived), and the app accent (accent-2 derived), with 13 one-click `COLOR_THEMES` (Developer/Classic/Neon/Pastel/Sunset/Space/Forest/Ocean/Cartographer/Aurora/Terracotta/Harbor/Orchard). New persisted flat `settings.palette` (`{ "--var": "#hex" }`, allow-listed via `THEMEABLE_VARS` + 6-digit-hex-validated, applied in both light/dark by `applyPaletteOverrides()` in `render()`; legacy `{light,dark}` folds to flat). Legend "Auto Colors" removed (the `>` key now opens the panel); editing a level color also re-renders notes; `usStateVisitMap.v1` schema otherwise unchanged. No active dev line. The prior **4.9.0 "Travel Agent"** (WISH-080): Rangefinder's summary row hands Start/End to an external trip tool — Drive opens the preferred provider's directions, Plane opens a Google Flights search, one point opens a provider place lookup; single click = focused new tab, right-click = background tab; the compare readout is now the Local Times opener and the `.ring-maps-btn` keeps a consistent icon; new persisted `settings.mapsProvider` (apple|google, platform-aware default) toggled in Settings → Map Interaction. No active dev line. The prior **4.8.1 "Reading Glasses"** persisted Text Only text size via `settings.notesTextOnlyFontPx` (defaulted in `defaultState()`, clamped 9–22px in `normalizeState()`); **4.8.0 "Square Deal"** (WISH-077) shipped the consistency pass. Anchored pop-ups (Priority, Grid, Rangefinder style/time, Legend position, Map/Waypoint label pickers, Waypoint priority/link, Basecamp row/icon/link) share one square-control system; controls share the 36px / 32px / 40px size system; desktop side-column Rangefinder stacks below Legend until the side reaches 40% width, then splits beside it; Rangefinder endpoint labels wrap inside their fixed-height slots; map actions stay top-aligned as modes change; Wayfinder is Notes-only. Shortcuts were fully revised: `|`/`:`/`9`–`6`/`'`/`←`/`~` for map, W/X/Y/Z + F/R + `>` (Auto Colors) for Legend/Notes, E/J/K/U/V + T (Time) for Rangefinder, numbered pop-up keys, and `{`/`}` for Text Only font size. Text Only floats a scroll-sticky Smaller/Larger/Copy cluster over its text surface (persisted `notesTextOnlyFontPx`, 9–22px). The next substantial change begins with `plan`, or `start` when a plan already exists.
...
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. No active dev line; the next substantial change begins with `plan`, or `start` when a plan already exists.
...
- User data lives in browser localStorage. Locate, Waypoint Pack Wikipedia
  ...
  online actions and only run when clicked.
