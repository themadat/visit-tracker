...
- Current version: `APP_VERSION = "5.1.1.1"` — active patch **Clear Signal**, adding the supplied Data Sync braces icon and useful browser-network failure messages. Latest cut: **5.1.0 Cloud Trail**, optional GitHub data sync from the sibling app-template. The combined local/cloud status action is first in the top-bar toolbar; Settings → Data Sync hosts setup, explicit transfer, conflict choices, cloud restore, recovery and payload preview. Fixed target: `themadat/app-data/main/data/visit-tracker.json`. Small direct patch; no plan doc. Connection root cause remains under investigation pending browser/channel details; no real credentials read or tested.
