...
- Current version: `APP_VERSION = "4.4.0.26"` — active development line is 4.4.0 "Basecamps"; latest cut release remains 4.3.0 "Waypoint Packs".
- 4.4.0.1 starts **Basecamps**: the original single scratchpad migrates once into a named pad, while new state stores up to 20 ordered pads with dedicated icons, sanitized rich-text HTML, derived plain text, timestamps, and links to existing US/World notes. The dialog now has responsive pad navigation, search with ephemeral matches, inline rename/icon controls, drag plus `Alt+Up`/`Alt+Down` reorder, rich formatting via Selection/Range APIs, safe paste/link handling, scoped copy/clear, linked-note search/open/unlink flows, and per-pad Markdown/RTF/Text exports. `usStateVisitMap.v1` is unchanged; old `{ text, updated }` Basecamp backups migrate additively.
- 4.4.0.26 polishes Basecamp formatting again: editor Heading is explicitly larger than Subheader, multi-line list insertion only replaces the whole block when the whole block is selected to avoid duplicated trailing lines, Font is seven buttons wide with size controls on the first row, Other now uses square-grid/link-badge/photo-badge/terminal/clear icons with disabled Image plus a working Code Block action, and WISH-073 tracks P2 Basecamp photo support. 4.4.0.25 fixes the prior Basecamp formatting follow-up: multi-line list conversion keeps selected text, editor-local Cmd/Ctrl+Z/Y undo/redo covers custom formatting mutations, checklist items start unchecked and can be toggled/exported, active toolbar buttons follow the current selection/cursor, selected text stays highlighted across font changes, Format/Font are capped at four columns, Text at six, and Link/Clear are stacked in an Other box. 4.4.0.24 turns Basecamp formatting into an icon-first system: desktop shows expansive Format/Font/Text icon groups, mobile uses compact dropdowns, new actions cover headings, dashed/bullet/numbered/check lists, font effects, size, baseline, alignment, justification, quote, and independent indent controls, and holding Cmd/Ctrl reveals shortcut chips for mapped formatting shortcuts. 4.4.0.23 keeps Basecamp's sidebar as a select/reorder-only pad list: row swipe edit/delete is removed, rows show larger titles with absolute last-updated timestamps, the active-row icon is nudged right, the editor header icon/title are larger, and header title/icon clicks handle rename/icon editing. New pads focus the header rename input, and picker icons inherit the same teal fill treatment as the sidebar/header. Earlier 4.4.0 builds also fixed World Locate scoping/projection and removed Basecamp's per-keystroke DOM normalization/full-document parsing, offscreen plain-text layout, and hidden Developer JSON rendering. WISH-071, optional latitude/longitude map lines, is now P0.
...
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. Active plan: [WISH-053 — Multiple Named Basecamp Pads with Rich Text](WISH-053-basecamp-pads-PLAN.md) (4.4.0 "Basecamps" line, branch `4-4-0-basecamps`). Active development is at `4.4.0.26`; `prep` and `ship` have not run.
...
- Basecamp: named rich-text pads with dedicated icons, cross-pad search, drag/keyboard reorder, safe URL/paste handling, linked existing notes, and active/all-pad copy.
