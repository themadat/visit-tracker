...
- Current version: `APP_VERSION = "4.6.0.4"` — active dev line **4.6.0 "The Whole Story"**. Plan: `context/4.6.0-DETAILED-NOTES-PLAN.md`. Latest cut release: 4.5.2 "Get Your Bearings".
  - **4.6.0 in progress:** Notes has a fourth `detailed` view before Expanded, using `__RECTANGLE_EXPAND_DIAGONAL`. Fresh/default settings use Detailed while existing saved view modes remain valid. Detailed cards use a level-colored location-precision/tag header, natural Location/Event phrases, compact direct priority choices, inline Details editing with a 2,000-character limit, and linked-Waypoint website/photo/priority actions in both Notes contexts. Coordinates are intentionally omitted. Detailed and Expanded location headers pair date span with a collapsible note count; Expanded badges stay on their owning Waypoint or first map icon. Selected-location Condensed rows are denser. Down Arrow cycles views, Up Arrow cycles sorts, Cmd/Ctrl+Enter saves Details, and Back restores the main list's scroll position. The same four-view selector renders in a sticky individual-location Visit Notes header and shares the persisted `notesViewMode`.
...
- Latest public releases (newest first): 4.5.2 "Get Your Bearings", 4.5.1 "Back in the Picture", 4.5.0 "Leave No Trace", 4.4.4 "Fine Print", 4.4.3 "Clear View", 4.4.2 "True Colors", 4.4.1 "Ultralight", 4.4.0 "Basecamps". Full per-release behavior lives in the `CHANGELOG` constant in `assets/js/changelog.js`; release table in `README.md`. Don't duplicate per-release prose here — read the CHANGELOG entry for the version in question.
