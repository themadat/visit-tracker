...
- Current version: `APP_VERSION = "4.7.5.3"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). Done: per-map `settings.graticuleByLayer` (`{us:{enabled,references,labels,major:10,minor:5}, world:{enabled,references,labels,major:30,minor:10}}`, defaults + `normalizeGraticuleByLayer`); a `renderGraticule()` injects a `#mapGraticule`/`#worldMapGraticule` `<g>` below the ring overlay and draws World (Robinson parallels/curved meridians + projection frame + clamped degree labels + optional named tropic/polar reference lines) and US (curved Albers-style projected graticule over the contiguous US frame; displaced AK/HI/territory insets intentionally do not get fake local grids). The header Grid button uses larger `__GLOBE` when off / `__GLOBE_FILL` when on and toggles a non-modal `#graticuleDialog` popover: Grid, Circles, Labels, Major, Minor in one compact row. Minor intervals snap to divisors of the major interval, with ArrowUp/ArrowDown on Minor cycling those divisors. Lines via `.graticule-line`/`.is-major`/`.graticule-frame`/`.graticule-label` (theme-aware). Latest cut release: **4.7.4 "Crosshairs"**. WISH-061 (Rangefinder edge cases) also targets 4.7.5 but is not yet started.
