...
- Current version: `APP_VERSION = "4.4.0.13"` — active development line is 4.4.0 "Basecamps"; latest cut release remains 4.3.0 "Waypoint Packs".
...
- 4.4.0.13 fixes World Locate by scoping Nominatim requests with ISO country codes, projects World pins and Rangefinder anchors with the Robinson projection used by the embedded 10°E-centered map, and removes Basecamp's per-keystroke DOM normalization/full-document parsing, offscreen plain-text layout, and hidden Developer JSON rendering. WISH-071, optional latitude/longitude map lines, is now P0.
...
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. Active plan: [WISH-053 — Multiple Named Basecamp Pads with Rich Text](WISH-053-basecamp-pads-PLAN.md) (4.4.0 "Basecamps" line, branch `4-4-0-basecamps`). Active development is at `4.4.0.13`; `prep` and `ship` have not run.
