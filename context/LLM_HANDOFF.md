...
- Open follow-ups: WISH-080 (5.1.0 Target Finder Maps/Flights integration, P0 — **planned**, see `context/WISH-080-TARGET-FINDER-MAPS-PLAN.md`); WISH-079 (mobile center-crosshair Rangefinder, P1); WISH-076 (native-language World labels, P1); WISH-073 (Basecamp photo support, P2).
- Plan docs live in `context/` only while their line is in flight, then are deleted on ship. No active dev line yet. Queued plans: `context/WISH-080-TARGET-FINDER-MAPS-PLAN.md` (Rangefinder external maps/flights handoff — summary-row re-wire: compare readout opens Local Times, the right button becomes a provider-logo websites button [click → new focused tab, right-click → background tab]; provider toggle in Settings → Map Interaction; Plane = Google Flights only; adds `settings.mapsProvider`; 5.1.0 minor) and `context/WISH-063-THEMING-OVERHAUL-PLAN.md` (app-wide theming overhaul — every color selectable, palette button moves to the top bar left of Settings, large grouped modal; 5.0.0 major flagship).
