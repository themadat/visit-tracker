...
Recent 3.1.0.13 surface note: Smart Convert placeholder text is visually quieter; the note date picker groups quick actions, date fields, and status text explicitly; Today/Year Only are equal-width grouped buttons spanning the same rhythm as three month chips; the date fields are centered between those quick actions and the Month/Day optional status text; the year stepper is tight; the date preview pill has a fixed height even with a two-line weekday; mobile month chips render three across; mobile Quick Add puts the location picker under the Add Note title/action row; Location Icon Tag search uses richer generated aliases, travel/activity synonyms, normalized multi-word query matching, broader animal/object/food/weather aliases, appliance/utility/mail/paper/boating/signpost aliases, and media/fuel/hand/head/hourglass/search/restroom/trash/trophy/storm/phone/TV/video/xmark-bin aliases; More Icons sorts aliased symbols before unaliased ones; and WISH-050 has been completed/removed from the roadmap seed.
