...
- Current version: `APP_VERSION = "5.1.1"` — released **Clear Signal**. Data Sync uses the supplied braces icon and clearer network-error messages; Theme & Colors now shares the toolbar's square control sizing. Fixed sync target: `themadat/app-data/main/data/trail-log.json`. The old filename matched EasyPrivacy's /visit-tracker.js rule, which also matches .json. Existing cloud users should rename the old file before syncing; the new target clears old comparison metadata while retaining local data and tokens. No active dev line or plan doc. No real credentials read or tested.
