...
- Current version: `APP_VERSION = "4.6.0.2"` — active dev line **4.6.0 "The Whole Story"**. Plan: `context/4.6.0-DETAILED-NOTES-PLAN.md`. Latest cut release: 4.5.2 "Get Your Bearings".
  - **4.6.0 in progress:** Notes has a fourth `detailed` view before Expanded, using `__RECTANGLE_EXPAND_DIAGONAL`. Fresh/default settings use Detailed while existing saved view modes remain valid. Detailed cards use a date/precision/level/tag header, paired Location and Event summaries, inline Details editing, and linked-Waypoint website/photo/priority actions. Coordinates are intentionally omitted. The same four-view selector renders in a sticky individual-location Visit Notes header and shares the persisted `notesViewMode`.
