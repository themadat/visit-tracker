...
- Current version: `APP_VERSION = "4.7.5.1"` — active dev line **4.7.5 "Gridlines"** implementing WISH-071 lat/lng graticule (plan `context/WISH-071-LATLNG-GRATICULE-PLAN.md`). Done: per-map `settings.graticuleByLayer` (`{us:{enabled,major:10,minor:5}, world:{enabled,major:30,minor:10}}`, defaults + `normalizeGraticuleByLayer`); a `renderGraticule()` injects a `#mapGraticule`/`#worldMapGraticule` `<g>` below the ring overlay and draws World (Robinson parallels/meridians + projection frame + major-line degree labels) and US (per-state/inset straight lat/lon lines from `LOCATION_GEO_BOUNDS`/`LOCATION_PROJECTION_FRAME_OVERRIDES`, no labels to avoid clutter); a header **Grid** button (`#mapGraticuleBtn`) opens `#graticuleDialog` (on/off + major/minor per active map). Lines via `.graticule-line`/`.is-major`/`.graticule-frame`/`.graticule-label` (theme-aware). Latest cut release: **4.7.4 "Crosshairs"**. WISH-061 (Rangefinder edge cases) also targets 4.7.5 but is not yet started.
