...
Recent 3.1.0.8 surface note: Smart Convert placeholder text is visually quieter; the note date picker groups quick actions, date fields, and status text explicitly; Today/Year Only are equal-width grouped buttons spanning the same rhythm as three month chips; the date fields plus status text span the six-chip rhythm beside them; the year stepper is tight; the date preview pill has a fixed height even with a two-line weekday; mobile month chips render three across; mobile Quick Add puts the location picker under the Add Note title/action row; and Location Icon Tag search now uses richer generated aliases, travel/activity synonyms, and normalized multi-word query matching.
