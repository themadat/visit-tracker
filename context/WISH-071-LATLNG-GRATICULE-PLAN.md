...
- **Manual tuning** — Developer Mode exposes separate Mainland and Alaska controls for horizontal/vertical offset, width, height, and latitude curve level. Longitude lines are mathematically constrained to straight chords. Tuning is saved in a dedicated local session key and exports a commit-ready `US_GRATICULE_CALIBRATION` constant.
...
- For the contiguous map, derive the grid from the landmark-fitted Albers projection, then straighten every longitude into a chord between its north/south anchors. Latitude lines retain a consistent shared curve that can be scaled without bending the longitude lines.
...
- Mainland and Alaska have independent X, Y, Width, Height, and Curve controls with live map updates.
- Session values use `trailLog.usGridTuning.v1`, outside `usStateVisitMap.v1`, so ordinary backups remain unchanged.
