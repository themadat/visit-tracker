...
6. US graticule: sampled National Atlas Equal Area overlay over the contiguous-US frame, drawn wider than the viewport; mask displaced insets out of the lower-48 projection, then draw an Alaska Albers grid and a Hawaii Mercator grid in their independent panes.
...
- US map: graticule on → contiguous US shows a National Atlas Equal Area lat/lng overlay with optional labels and lines continuing off all four SVG edges; Alaska shows an Albers inset grid and Hawaii a Mercator inset grid fitted to their separator-pane containers; territories remain masked out; off → removed.
