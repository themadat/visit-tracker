...
`4.7.1.12` is open. The first pass added offline English short names, shrink-to-fit candidates, greedy collision filtering, and leader-line callouts for `.circlexx` microstate dots. Follow-up passes lowered the maximum Name label size, allowed multi-word names to wrap, and kept leader lines dot-only. Recent adjustments shift toward atlas-density completeness: all shape-backed country labels render over their region at tiny scale, dot anchors include `.circlexx`, `.subxx`, and `.unxx`, callouts clamp inside the World viewBox, helper classes like `eu` are ignored, nonstandard regions like Crimea, Southern Kuril Islands, and Svalbard have explicit names, dense island clusters get hand-tuned offsets, and tiny UK subregion labels render inside the United Kingdom. Latest work adds a World/Name Tune mode: drag rendered labels, nudge the selected label with arrow keys, edit selected label text/size, keep leader lines following moved callouts, export merged manual overrides as `assets/js/world-label-overrides.js`, dismiss the tuner from the map, restore it from Developer Tools, preserve real diacritics in names, make exports explicit about merging the loaded override file plus session edits for repeated passes, normalize all World Name font sizes to the supported 8/12/16 choices, expose those sizes as buttons, and export explicit `lines` overrides so manual names can force or remove multiline wrapping. Parse/diff checks pass; re-run in-app browser smoke on port 8018 before ship.
