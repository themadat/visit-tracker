...
- **`touch-action` / browser gesture capture (the big one):** `.map-wrap` uses native vertical scrolling, and scroll mode allows native horizontal panning with `touch-action: pan-x pan-y`; the JS pan lock waits for clear horizontal intent before grabbing touch movement. Small-screen scroll mode locks the map viewport to the active SVG's 100% height so pinch zoom expands pannable content instead of resizing the page section. Avoid synthetic page scrolling on pointermove. Also guard iOS Safari's non-standard `gesturestart/gesturechange` (preventDefault) if they still fire.
